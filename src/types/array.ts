...
export default class InternalArray implements ValidateType {
  static message: Obj = {
  ...
  validateTypes() {
    if (!rule.type(this._value, "array")) {
      this.error.push(messageFormat(InternalArray.message.type, this._field));
    }
  }
  validateRequired() {
    if (this._rule.required && !rule.required(this._value)) {
      this.error.push(
        messageFormat(InternalArray.message.required, this._field)
      );
    }
  }
}