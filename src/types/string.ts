...
export default class InternalString implements ValidateType {
  static message: Obj = {
  ...
  validateTypes() {
    if (!rule.type(this._value, "string")) {
      this.error.push(messageFormat(InternalString.message.type, this._field));
    }
  }
  validateRequired() {
    if (this._rule.required && !rule.required(this._value)) {
      this.error.push(
        messageFormat(InternalString.message.required, this._field)
      );
    }
  }
}
