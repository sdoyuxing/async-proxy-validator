...
export default class InternalUrl implements ValidateType {
  static message: Obj = {
  ...
  validateTypes() {
    if (!rule.type(this._value, "url")) {
      this.error.push(messageFormat(InternalUrl.message.type, this._field));
    }
  }
  validateRequired() {
    if (this._rule.required && !rule.required(this._value)) {
      this.error.push(messageFormat(InternalUrl.message.required, this._field));
    }
  }
}
